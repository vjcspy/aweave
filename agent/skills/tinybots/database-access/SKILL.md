...
The project uses MySQL databases running via Docker Compose (`workspaces/devtools/tinybots/local/docker-compose.yaml`):
...
```bash
cd workspaces/devtools/tinybots/local && docker compose up -d mysql-typ-e-db mysql-wonkers-db
```

...
Your goal is to audit the workspace to ensure the **TinyBots Global Overview** (`resources/workspaces/tinybots/OVERVIEW.md`) is structurally perfect, accurately reflects the current repository landscape, and preserves critical operational guidance already present in the document.
...
2. **Overview Status Check:**
   - For each discovered repository, calculate the expected path: `resources/workspaces/tinybots/<repo>/OVERVIEW.md`.
   - *Check:* Does this file exist? (Status: Present / Missing).
3. **Existing Global Overview Extraction (Mandatory):**
   - Read the existing `resources/workspaces/tinybots/OVERVIEW.md` before making changes.
...
The `resources/workspaces/tinybots/OVERVIEW.md` file **MUST** strictly adhere to the following Heading Hierarchy (H2/H3) in exact order:
...
1. Operational content preservation:
   ...
   - Testing constraints and the canonical `just -f workspaces/devtools/tinybots/local/Justfile test-<repo>` pattern are still present.
2. Schema compliance:

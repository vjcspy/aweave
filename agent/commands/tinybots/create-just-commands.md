...
Infrastructure này nằm tại `workspaces/devtools/tinybots/local/` (không còn ở `workspaces/devtools/` root).
...
```bash
just -f workspaces/devtools/tinybots/local/Justfile start-atlas 2>&1 | head -50
```
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│                 workspaces/devtools/tinybots/local/docker-compose.yaml                   │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
...
When migrating from `workspaces/tinybots/backend/<repo>/ci/docker-compose.yml` to `workspaces/devtools/tinybots/local/docker-compose.yaml`:
...
```
workspaces/tinybots/backend/<repo>/ci/docker-compose.yml    # Service defs & dependencies
workspaces/tinybots/backend/<repo>/ci/test.sh               # Startup sequence & cleanup logic
workspaces/tinybots/backend/<repo>/ci/local-test.sh         # Wrapper (if exists)
workspaces/tinybots/backend/<repo>/ci/node-verify.sh        # Entrypoint (if exists)
```
...
Check if the test runner service already exists in `workspaces/devtools/tinybots/local/docker-compose.yaml`:

**Important**: Trong `docker-compose.yaml`, **chỉ dùng relative paths** cho `volumes:` (ví dụ `../../../tinybots/...`). Không dùng absolute paths để đảm bảo portable giữa các máy.

```yaml
  <repo-name>:
    image: node:22-alpine
    volumes:
      - ../../../tinybots/backend/<repo-name>:/usr/src/app
    labels:
      - <repo-name>
    environment:
...
Create `workspaces/devtools/tinybots/local/justfiles/<repo-name>.just`:
...
Add to `workspaces/devtools/tinybots/local/Justfile`:
...
```yaml
  micro-manager:
    image: node:22-alpine
    volumes:
      - ../../../tinybots/backend/micro-manager:/usr/src/app
    labels:
      - micro-manager
    environment:
      DB_RW_HOST: mysql-typ-e-db
...
Create `workspaces/devtools/tinybots/local/justfiles/micro-manager.just`:
...
docker compose -f workspaces/devtools/tinybots/local/docker-compose.yaml logs -f <service>

# Get shell into running container
docker compose -f workspaces/devtools/tinybots/local/docker-compose.yaml exec <service> sh

# Force recreate containers
docker compose -f workspaces/devtools/tinybots/local/docker-compose.yaml up -d --force-recreate <service>

# Remove all devtools containers
docker compose -f workspaces/devtools/tinybots/local/docker-compose.yaml down
```
...
- [ ] Read `workspaces/tinybots/backend/<repo>/ci/docker-compose.yml`
- [ ] Read `workspaces/tinybots/backend/<repo>/ci/test.sh`
...
- [ ] Verify test runner service exists in `workspaces/devtools/tinybots/local/docker-compose.yaml`
- [ ] If not, add service definition with corrected hostnames
- [ ] Create `workspaces/devtools/tinybots/local/justfiles/<repo>.just` with standard commands
- [ ] Add import to `workspaces/devtools/tinybots/local/Justfile`
...
| File | Purpose |
|------|---------|
| `workspaces/devtools/tinybots/local/Justfile` | Main orchestration file |
| `workspaces/devtools/tinybots/local/docker-compose.yaml` | Complete service definitions |
| `workspaces/devtools/tinybots/local/justfiles/*.just` | Per-repo command files |
| `resources/workspaces/devtools/tinybots/OVERVIEW.md` | TinyBots DevTools overview |
| `resources/workspaces/tinybots/OVERVIEW.md` | Global TinyBots overview (repos + testing) |
| `workspaces/tinybots/backend/<repo>/ci/docker-compose.yml` | Original repo CI configs (reference) |
| `workspaces/tinybots/backend/<repo>/ci/test.sh` | Original test scripts (reference) |

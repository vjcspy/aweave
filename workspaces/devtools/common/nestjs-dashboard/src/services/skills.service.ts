...
export class SkillsService {
  ...
  async getAllSkills(): Promise<SkillItem[]> {
    ...
    if (fs.existsSync(globalSkillsDir)) {
      skills.push(...this.scanSkillsDirectory(globalSkillsDir, globalSkillsDir));
    }
  ...
  async setSkillActive(skillId: string, active: boolean): Promise<void> {
    ...
    fs.writeFileSync(this.activeSkillsPath, JSON.stringify({ active: newActiveArray }, null, 2), 'utf8');

    await this.generateLoadedSkillsMd();
  }

  private scanSkillsDirectory(baseDir: string, rootDirForId: string): SkillItem[] {
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          ...
          if (fs.existsSync(skillMdPath)) {
            const skill = this.parseSkillFrontmatter(skillMdPath, dirPath, rootDirForId);
            if (skill) {
              results.push(skill);
            }
          }
  ...
  private parseSkillFrontmatter(skillMdPath: string, dirPath: string, rootDirForId: string): SkillItem | null {
    try {
      const content = fs.readFileSync(skillMdPath, 'utf8');
      const parsed = matter(content);
      if (!parsed.data.name || !parsed.data.description) {
        this.logger.warn(`SKILL.md at ${skillMdPath} is missing name or description in frontmatter.`);
  ...
  private async generateLoadedSkillsMd(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Generated active skills context at ${this.loadedSkillsMdPath}`);
    } catch (e) {

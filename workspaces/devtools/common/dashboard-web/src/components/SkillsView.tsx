...
import { Cpu, Loader2, Power, Code2, ShieldAlert, Sparkles, Zap, Waypoints } from 'lucide-react';
    ...
    <div className="flex flex-col h-full animate-in fade-in duration-500">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Enable or disable specialized AI sub-agents to enhance your workspace contextual understanding.</p>
        </div>
      ...
      {loading && skills.length === 0 ? (
        ...
        <div className="glass-panel rounded-xl p-12 text-center flex flex-col items-center">
          ...
          <p className="text-zinc-500 max-w-md">
            The workspace didn't return any skills. Make sure you have valid `SKILL.md` files in `agent/skills/` or `~/.aweave/skills/`.
          </p>
        </div>
      ) : (
          ...
          {skills.map((skill) => (
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                <div className={`p-3 rounded-xl inline-flex ${skill.active ? 'bg-purple-500/20 text-purple-300' : 'bg-white/5 text-zinc-400'}`}>
                  {skill.active ? <Sparkles className="w-6 h-6" /> : <Waypoints className="w-6 h-6" />}
                </div>
                ...
                <button
                  ...
                  className={`relative flex items-center h-7 w-14 rounded-full transition-all duration-300 focus:outline-none overflow-hidden ${
                    skill.active ? 'bg-purple-600 shadow-[0_0_10px_rgba(168,85,247,0.6)]' : 'bg-zinc-700/80 border border-white/5 hover:bg-zinc-600'
                ...
                >
                  ...
                  >
                    {toggling === skill.id ? (
                      ...
                      <Power className="w-3 h-3 text-purple-600" strokeWidth={3} />
                    ) : null}
                  </span>
                </button>
              </div>
              ...
              <div className="relative z-10 flex-1">
                <h3 className={`text-lg font-bold tracking-tight mb-2 ${skill.active ? 'text-white' : 'text-zinc-200'}`}>
                  {skill.name}
                </h3>

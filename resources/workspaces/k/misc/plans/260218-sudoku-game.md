...
- Thay đổi homepage `workspaces/k/misc/git-relay-vercel` thành game giải đố Sudoku
...
```
workspaces/k/misc/git-relay-vercel/
...
1. **Chạy dev server:**
   ...
   ```bash
   cd workspaces/k/misc/git-relay-vercel && pnpm dev
   ```
...
3. **Build check:**
   ...
   ```bash
   cd workspaces/k/misc/git-relay-vercel && pnpm build
   ```

...
- **cli-shared:** Pure utility library for the entire `aw` CLI ecosystem — MCP response format (AI agent contract), HTTP client (native fetch), output helpers, content reading (`--file`/`--content`/`--stdin`), shared pino logger (`getCliLogger()`), and native process manager for server daemon lifecycle. Depends on `@hod/aweave-node-shared`. Leaf dependency consumed by all plugins ([resources/workspaces/devtools/common/cli-shared/OVERVIEW.md](resources/workspaces/devtools/common/cli-shared/OVERVIEW.md))
...
- **node-shared:** Node.js runtime utilities shared across all layers — (1) standardized DevTools root discovery (`env → cwd → moduleDir`) via marker-based walk-up, and (2) shared pino logger factory (`createLogger()`) with file rotation (`pino-roll`), error-level splitting, and MCP-safe stderr-only console output. Used by `nestjs-core` (async mode, pino-roll rotation) and `cli-shared`/`getCliLogger()` (sync mode for short-lived processes). ([resources/workspaces/devtools/common/node-shared/OVERVIEW.md](resources/workspaces/devtools/common/node-shared/OVERVIEW.md))
...
- **nestjs-core:** Shared NestJS infrastructure module (`@Global`) — structured JSON logging via pino (backed by `node-shared`'s `createLogger()`; log files: `~/.aweave/logs/server.jsonl` + `server.error.jsonl` with daily rotation; colorized pino-pretty console on **stderr** in dev), AsyncLocalStorage-based request context propagation, HTTP `x-correlation-id` middleware with auto-generation. Consumed by all feature modules without explicit imports ([resources/workspaces/devtools/common/nestjs-core/OVERVIEW.md](resources/workspaces/devtools/common/nestjs-core/OVERVIEW.md))
...
- **Build order:** `node-shared` → `cli-shared` → shared libraries (`debate-machine`, `workflow-engine`, `playwright`, `config-core`) → `nestjs-core` → NestJS feature modules (`nestjs-debate`, `nestjs-dashboard`) → `server` → plugins → `cli` (last).
...
- **Structured logging:** All NestJS modules use pino-backed structured JSON logging via `nestjs-core`. All CLI commands use `getCliLogger()` from `cli-shared`. Both are backed by `createLogger()` from `node-shared`. Log files: `~/.aweave/logs/server.jsonl` + `server.error.jsonl` (server) and `cli.jsonl` + `cli.error.jsonl` (CLI) with daily rotation. Console output always goes to **stderr** — stdout never receives pino output (MCP stdio safety).
- **SQLite databases:**

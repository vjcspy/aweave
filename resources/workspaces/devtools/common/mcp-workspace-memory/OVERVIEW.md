...
- **Global binary** (`aw-mcp-memory`)
  - Mapped via `package.json#bin` to `dist/stdio.js`
  - Intended for agent/client configs that prefer stable command names over raw `node <path>`

...
- Current debate-server plan: `resources/workspaces/devtools/common/_plans/260131-debate-server.md`
...
- Current NestJS server (init): `workspaces/devtools/common/server/`
- Current debate-server (to be replaced): `workspaces/devtools/common/debate-server/`
- Debate CLI: `workspaces/devtools/common/cli/devtool/aweave/debate/`
- Prisma v7 SQLite docs: https://www.prisma.io/docs/orm/overview/databases/sqlite

## User Requirements

1. Replace per-feature standalone servers with a **single unified NestJS server** at `workspaces/devtools/common/server/`
...
Restructure the devtools Node.js backend from standalone per-feature servers (e.g. `debate-server`) to a unified NestJS server with modular feature packages. Each feature becomes a separate pnpm workspace package (`@hod/aweave-nestjs-<feature>`) exporting a NestJS module. The common server (`workspaces/devtools/common/server/`) imports all feature modules and provides shared infrastructure (auth, config, error handling).
...
7. **Services management update** — CLI `services.py` needs path updates from `workspaces/devtools/common/debate-server/` to `workspaces/devtools/common/server/`. The pm2 ecosystem config moves to the common server.

## 📐 Spec / Decisions

### 1. Package Naming & Location

**Convention:** `@hod/aweave-nestjs-<feature>` for package name, `workspaces/devtools/<domain>/nestjs-<feature>/` for folder.

| Package | Folder | Database |
|---------|--------|----------|
| `@hod/aweave-nestjs-debate` | `workspaces/devtools/common/nestjs-debate/` | `~/.aweave/db/debate.db` |
| `@hod/aweave-server` | `workspaces/devtools/common/server/` | N/A (imports modules) |
| Future: `@hod/aweave-nestjs-<x>` | `workspaces/devtools/<domain>/nestjs-<x>/` | `~/.aweave/db/<x>.db` |
...
**`workspaces/devtools/common/cli/devtool/aweave/debate/config.py` changes:**
```python
# Remove:
...
**`workspaces/devtools/common/cli/devtool/aweave/debate/cli.py` — `wait` command changes:**
```python
...
**`workspaces/devtools/common/cli/devtool/aweave/debate/services.py` changes:**
...
The main server (`workspaces/devtools/common/server/`) provides:
...
- [ ] Verify NestJS server at `workspaces/devtools/common/server/` is correctly initialized and builds
  - **Outcome**: `pnpm build` succeeds in `workspaces/devtools/common/server/`
...
- [ ] Create `workspaces/devtools/common/nestjs-debate/` directory
...
- [ ] Update `workspaces/devtools/pnpm-workspace.yaml` to include `common/nestjs-debate`
  - **Outcome**: `pnpm install` from `devtools/` resolves workspace links

#### Step 2: Prisma Schema & Client Generation

- [ ] Create `workspaces/devtools/common/nestjs-debate/prisma/schema.prisma` (see Spec section 2)
...
- [ ] Run `pnpm build` in `workspaces/devtools/common/nestjs-debate/` to verify compilation
  - **Outcome**: Package builds successfully

#### Step 8: Update Main Server

- [ ] Add `@hod/aweave-nestjs-debate` as dependency in `workspaces/devtools/common/server/package.json`:
  ```json
  "dependencies": {
...
- [ ] Update `workspaces/devtools/common/cli/devtool/aweave/debate/config.py`:
  - Remove `POLL_TIMEOUT = 65`
  - Add `POLL_INTERVAL = float(os.getenv("DEBATE_POLL_INTERVAL", "2"))`
  - **Outcome**: Config reflects interval polling

- [ ] Update `workspaces/devtools/common/cli/devtool/aweave/debate/cli.py`:
...
- [ ] Update `workspaces/devtools/common/cli/devtool/aweave/debate/services.py`:
  - Change `DEBATE_SERVER_DIR` to point to `workspaces/devtools/common/server/`
...
- [ ] Start server: `cd workspaces/devtools/common/server && pnpm start`
...
- [ ] Update `workspaces/devtools/pnpm-workspace.yaml` — remove `common/debate-server` if no longer needed
- [ ] Add deprecation notice to `workspaces/devtools/common/debate-server/README.md`
- [ ] Update `resources/workspaces/devtools/OVERVIEW.md` — reflect new server architecture
- [ ] Update `resources/workspaces/devtools/common/_plans/debate.md` section 2.8 — reflect new server location and architecture

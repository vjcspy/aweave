...
- Viết 1 package trong `resources/workspaces/devtools/common` có nhiệm vụ quản lý các config, dùng được ở cả NestJS, Next.js và CLI.
...
resources/workspaces/devtools/common/config-core/
└── OVERVIEW.md                              # 🚧 TODO - documentation
```

#### Monorepo Wiring Tasks

- [x] Register new packages in `workspaces/devtools/pnpm-workspace.yaml`:
  - `workspaces/devtools/common/config-core`
  - `workspaces/devtools/common/config`
...
- [x] Add `@hod/aweave-plugin-config` as dependency in `workspaces/devtools/common/cli/package.json` + register in `oclif.plugins`
...
3. **Tạo package config per-domain**
   - `@hod/aweave-config-common` tại `workspaces/devtools/common/config/`
...
4. **Setup flow & CLI command ownership**
   ...
   **Package:** `workspaces/devtools/common/cli-plugin-config/`

   ```
   workspaces/devtools/common/cli-plugin-config/          # 🚧 TODO - new oclif plugin
   ├── package.json                            # @hod/aweave-plugin-config
   ├── tsconfig.json
   └── src/
       └── commands/
   ...
   **Dependencies:**
   ...
   - [x] Add `workspaces/devtools/common/cli-plugin-config` to `workspaces/devtools/pnpm-workspace.yaml`
   - [x] Add dependency in `workspaces/devtools/common/cli/package.json`:
     ```json
     "@hod/aweave-plugin-config": "workspace:*"
     ```
   - [x] Register in oclif plugins (`workspaces/devtools/common/cli/package.json` → `oclif.plugins`):
     ```json
...
6. **Docs**
   - Viết `resources/workspaces/devtools/common/config-core/OVERVIEW.md` (API, examples, layout config, precedence rules, projection contract).
...
#### 1. `@hod/aweave-config-core` — `workspaces/devtools/common/config-core/`
...
#### 2. `@hod/aweave-plugin-config` — `workspaces/devtools/common/cli-plugin-config/`
...
- `@hod/aweave-nab-config` (`devtools/nab/nab-config/`) — domain config package for nab
    ...
    - See: `resources/workspaces/devtools/nab/_plans/260210-clm-config-extraction.md`

### Not Implemented (Deferred)

- `@hod/aweave-config-common` (`workspaces/devtools/common/config/`) — domain config package with default YAML files
- `resources/workspaces/devtools/common/config-core/OVERVIEW.md` — documentation

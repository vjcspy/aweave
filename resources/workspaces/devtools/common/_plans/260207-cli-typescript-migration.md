...
> See `resources/workspaces/devtools/common/_plans/260207-cli-oclif-refactor.md` for the current architecture.
...
- Debate ecosystem spec: `resources/workspaces/devtools/common/_plans/debate.md`
- Unified NestJS server plan: `resources/workspaces/devtools/common/_plans/260207-unified-nestjs-server.md`
- Debate server plan: `resources/workspaces/devtools/common/_plans/260131-debate-server.md`
- Docs CLI plan: `resources/workspaces/devtools/common/_plans/260131-docs-cli-tool.md`
- Bitbucket CLI implementation: `resources/workspaces/devtools/nab/plugin-nab-confluence/OVERVIEW.md`
- Auto-start services plan: `resources/workspaces/devtools/common/_plans/260204-debate-auto-start-services.md`
- Python MCP response: `workspaces/devtools/common/cli/devtool/aweave/mcp/response.py`
- Python HTTP client: `workspaces/devtools/common/cli/devtool/aweave/http/client.py`
- Python debate CLI: `workspaces/devtools/common/cli/devtool/aweave/debate/cli.py`
- Python debate config: `workspaces/devtools/common/cli/devtool/aweave/debate/config.py`
- Python debate services: `workspaces/devtools/common/cli/devtool/aweave/debate/services.py`
- Python core entrypoint: `workspaces/devtools/common/cli/devtool/aweave/core/main.py`
...
Migrate the entire `aw` CLI toolset from Python (Typer) to TypeScript (Commander.js), organized as pnpm workspace packages. After migration, the Python CLI (`workspaces/devtools/common/cli/devtool/`) is deleted and the `aw` command runs from Node.js globally installed via pnpm.
...
|---------|----------|--------|--------------|
| CLI Core | `@hod/aweave` | `workspaces/devtools/common/cli-core/` | `commander` |
| Debate CLI | `@hod/aweave-debate` | `workspaces/devtools/common/cli-debate/` | `@hod/aweave` |
| Docs CLI | `@hod/aweave-docs` | `workspaces/devtools/common/cli-docs/` | `@hod/aweave`, `better-sqlite3` |
...
```typescript
// === @hod/aweave — workspaces/devtools/common/cli-core/src/program.ts ===
...
// === @hod/aweave — workspaces/devtools/common/cli-core/src/bin/aw.ts ===
#!/usr/bin/env node
import { program } from '../program.js';
program.parse();

// === @hod/aweave-debate — workspaces/devtools/common/cli-debate/src/commands.ts ===
import { Command } from 'commander';
...
```typescript
// workspaces/devtools/common/cli-core/src/mcp/response.ts
...
```typescript
// workspaces/devtools/common/cli-core/src/http/client.ts
...
```jsonc
// workspaces/devtools/common/cli-core/package.json
{
  "name": "@hod/aweave",
  "version": "0.1.0",
  "bin": {
...
```bash
# During development (from workspaces/devtools/common/cli-core/)
pnpm build
...
- [ ] Create `workspaces/devtools/common/cli-core/` directory
- [ ] Create `package.json`:
  - name: `@hod/aweave`
...
- [ ] Update `workspaces/devtools/pnpm-workspace.yaml`:
  - Add `common/cli-core`
...
- [ ] Create `workspaces/devtools/common/cli-debate/` directory
- [ ] Create `package.json`:
  - name: `@hod/aweave-debate`
  - dependencies: `@hod/aweave` (workspace:*), `commander`
  - **Outcome**: Package initialized
- [ ] Create `tsconfig.json` (same pattern as cli-core)
- [ ] Update `workspaces/devtools/pnpm-workspace.yaml`: add `common/cli-debate`
...
- [ ] Create `src/services.ts` — Port from Python `debate/services.py`:
  ...
  - Path: `workspaces/devtools/common/server/` (unified NestJS server, NOT old debate-server)
  - pm2 ecosystem config: `workspaces/devtools/common/server/ecosystem.config.cjs`
...
- [ ] Update `workspaces/devtools/common/cli-core/src/program.ts`:
  ```typescript
  import { debateCommand } from '@hod/aweave-debate';
  program.addCommand(debateCommand);
  ```
...
- [ ] Create `package.json`:
  ...
  - **Outcome**: Package initialized
- [ ] Create `tsconfig.json`
- [ ] Update `workspaces/devtools/pnpm-workspace.yaml`: add `nab/plugin-nab-confluence`
...
- [ ] Create `workspaces/devtools/common/cli-docs/` directory
- [ ] Create `package.json`:
  ...
  - **Outcome**: Package initialized
- [ ] Create `tsconfig.json`
- [ ] Update `workspaces/devtools/pnpm-workspace.yaml`: add `common/cli-docs`
...
- [ ] Delete `workspaces/devtools/common/cli/devtool/` directory (entire Python CLI)
...
- [ ] Delete `workspaces/devtools/scripts/generate-registry.py` (Node plugin registry no longer needed)
- [ ] Delete `workspaces/devtools/scripts/install-all.sh` (or rewrite for TypeScript-only)
  - **Outcome**: No Python CLI code remains

#### Step 20: Update Documentation

- [ ] Update `resources/workspaces/devtools/OVERVIEW.md`:
...
- [ ] Update `resources/workspaces/devtools/common/_plans/debate.md` section 2.3:

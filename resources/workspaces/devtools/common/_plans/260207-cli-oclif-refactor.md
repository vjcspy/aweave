...
- Previous migration plan (superseded): `resources/workspaces/devtools/common/_plans/260207-cli-typescript-migration.md`
...
|---------|----------|--------|--------|
| CLI Shared | `@hod/aweave-cli-shared` | `workspaces/devtools/common/cli-shared/` | ✅ Complete — pure utility library, no framework deps |
| CLI Main | `@hod/aweave` | `workspaces/devtools/common/cli/` | ✅ Complete — oclif app with `bin/run.js`, plugins declared |
| Debate Plugin | `@hod/aweave-plugin-debate` | `workspaces/devtools/common/cli-plugin-debate/` | ✅ Complete — 11 commands under `aw debate *` |
| Docs Plugin | `@hod/aweave-plugin-docs` | `workspaces/devtools/common/cli-plugin-docs/` | ✅ Complete — 7 commands under `aw docs *` |
...
- Old `cli-core/` package: **Deleted** (no longer in `workspaces/devtools/common/`)
...
### oclif Config (in `workspaces/devtools/common/cli/package.json`)
...
1. `cd workspaces/devtools && pnpm -r build` — rebuild all packages
2. `cd workspaces/devtools/common/cli && pnpm link --global` — re-link global binary
3. `aw --version` — verify recovery
4. If plugin discovery fails: check `oclif.plugins` in `workspaces/devtools/common/cli/package.json` matches installed packages

## 📐 Package Structure

| Package | npm name | Folder | Dependencies |
|---------|----------|--------|--------------|
| CLI Shared | `@hod/aweave-cli-shared` | `workspaces/devtools/common/cli-shared/` | (none) |
| CLI Main | `@hod/aweave` | `workspaces/devtools/common/cli/` | `@oclif/core`, `cli-shared`, all plugins |
| Debate Plugin | `@hod/aweave-plugin-debate` | `workspaces/devtools/common/cli-plugin-debate/` | `@oclif/core`, `cli-shared` |
| Docs Plugin | `@hod/aweave-plugin-docs` | `workspaces/devtools/common/cli-plugin-docs/` | `@oclif/core`, `cli-shared`, `better-sqlite3` |
...
- Package: `workspaces/devtools/common/cli-shared/`, no external deps
...
- [ ] Verify `resources/workspaces/devtools/OVERVIEW.md` matches current oclif architecture
- [ ] Verify `resources/workspaces/devtools/common/cli/OVERVIEW.md` references oclif patterns
- [ ] Verify `resources/workspaces/devtools/common/cli-plugin-debate/OVERVIEW.md` is current
- [ ] Verify `resources/workspaces/devtools/common/cli-plugin-docs/OVERVIEW.md` is current
- [ ] Verify `agent/rules/common/workspaces/devtools.md` references oclif (not Commander)
...
```bash
cd workspaces/devtools/common/cli
pnpm link --global

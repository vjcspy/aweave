...
```bash
cd workspaces/devtools/tinybots/local
...
```bash
cd workspaces/devtools/tinybots/local && npm run generate && npm run seed:clean && npm run seed -- --scope=micro-manager-triggered
```

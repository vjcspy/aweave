...
- `tinybots/backend/megazord-events/src/models/domains/SubscriptionDomain.ts` - Domain model with problematic `@Exclude()` decorator
- `tinybots/backend/megazord-events/src/services/EventSubscriptionService.ts` - Service using queue field for message routing
- `tinybots/backend/megazord-events/src/controllers/EventSubscriptionsController.ts` - Controller exposing subscription endpoints
- `resources/workspaces/tinybots/backend/megazord-events/251210-Queue-Per-Subscription.md` - Original refactor plan
- `resources/workspaces/tinybots/backend/megazord-events/OVERVIEW.md` - Repository overview
...
2. **Serialization Strategy**: Document class-transformer usage patterns in `resources/workspaces/tinybots/megazord-events/OVERVIEW.md` for future reference
